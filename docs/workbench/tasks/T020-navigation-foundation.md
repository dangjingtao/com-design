...
- Review fixes:
  ...
  - Codex 对 PR #35 继续发现 2 个 canonical bypass：accessibility invariant schema 约束不足、非字符串 sample-tree icon 可绕过 T013 strict lookup。两项由 follow-up PR #36 收紧 schema + procedural validator，并新增 malformed icon / contradictory accessibility regression。

## Review

- Reviewer: Mira
- Result: PASS
- Conclusion: T020 满足全部验收条件，并经 PR #35 / #36 完成 AI review follow-up hardening。Navigation Foundation 已成为 canonical machine contract，并进入 Canonical Design Model、Validation Evidence 与 Agent Contract；递归、多端 presentation、Host Chrome 边界及 input/a11y 均有真实 machine evidence。Preview 交互证据现使用可键盘操作的 link/button，且 validator 在 T013 dependency 非法时仍返回可组合错误。实现没有把微信 Capsule、App Shell 或未经验证的 Side Navigation API 误升为 Core Component。
