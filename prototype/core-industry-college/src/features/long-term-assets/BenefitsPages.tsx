...
export function BenefitsPage() {
  ...
  }), [benefitStatusFor, filter, loggedIn]);
  return <PublicShell><PageHeader title="权益" subtitle="统一表达平台、赛事、企业与活动来源" /><div className="space-y-5 px-4 py-5"><Card className="border border-border-subtle"><p className="text-sm leading-5 text-text-secondary">权益是成长与活动的支撑能力，不是商城。每一项都说明来源与资格依据；账号状态登录后读取。</p></Card>{loggedIn && <div className="flex gap-2">{(["all","available","history"] as const).map(value => <button key={value} onClick={() => setFilter(value)} className={`min-h-10 rounded-full px-3 text-sm font-medium ${filter === value ? "bg-primary-container text-text-brand" : "bg-surface text-text-secondary"}`}>{value === "all" ? "全部" : value === "available" ? "可用" : "历史"}</button>)}</div>}<div className="space-y-3">{visible.map(item => { const status = loggedIn ? benefitStatusFor(item.id) : undefined; return <Link to={`/benefits/${item.id}`} key={item.id} className="block"><Card interactive className="space-y-3"><SourceLine source={item.source} /><div className="flex items-start justify-between gap-3"><div><h2 className="font-semibold text-text-primary">{item.title}</h2><p className="mt-1 text-sm leading-5 text-text-secondary">{item.summary}</p></div>{status ? <StatusTag tone={benefitTone(status)}>{benefitLabel[status]}</StatusTag> : <StatusTag tone="neutral">登录查看资格</StatusTag>}</div>{item.expiresAt && <p className="text-xs text-text-tertiary">有效期至 {item.expiresAt}</p>}</Card></Link>; })}</div>{loggedIn ? <Link to="/benefits/wallet" className="block min-h-touch rounded-control bg-primary-container px-4 py-3 text-center text-sm font-medium text-text-brand">查看我的权益记录</Link> : <Card><p className="text-sm text-text-secondary">登录后可查看领取、使用与历史权益记录。</p></Card>}</div></PublicShell>;
}
