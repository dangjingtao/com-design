...
test("standalone registration portal completes leader flow on desktop", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "三创赛报名" })).toBeVisible();

  await page.getByRole("button", { name: /我是队长/ }).click();
  await expect(page.getByRole("heading", { name: "三创队长注册" })).toBeVisible();
  await page.getByRole("button", { name: "注册并进入答题" }).click();

  await expect(page.getByRole("heading", { name: "三创注册答题" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "团队信息" })).toBeVisible();

  await page.getByRole("button", { name: "添加成员" }).click();
  await expect(page.getByRole("heading", { name: "添加团队成员" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "承诺书" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "团队业绩报告" })).toBeVisible();
...
test("standalone registration portal keeps member branch usable on mobile", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "三创队员注册" })).toBeVisible();
  await page.getByRole("button", { name: "注册并进入答题" }).click();
  await page.getByRole("button", { name: /B\. 等待审核结果/ }).click();
  await page.getByRole("button", { name: "提交答题" }).click();
