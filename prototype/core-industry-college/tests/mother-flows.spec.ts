...
test("A guest can browse public competition and login back into registration", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "赛事" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "赛事报名" })).toBeVisible();
});
...
test("B registration returns pending, approval and workspace access on one account", async ({ page }) => {
  ...
  await expect(page.getByText("报名已提交，等待学校审核真实性")).toBeVisible();
  await page.getByRole("button", { name: "模拟审核通过" }).click();
  await expect(page.getByRole("heading", { name: "赛事工作区" })).toBeVisible();
  await expect(page.getByText(/身份：active/)).toBeVisible();
});
...
test("C workshop task keeps task identity through answer, generation and result", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "任务答题" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "生成确认" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "成果详情" })).toBeVisible();
...
test("D opportunity-company-resume-returnTo-application stays continuous", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "机会详情" })).toBeVisible();
  await page.getByRole("button", { name: /北辰美妆 · 查看企业/ }).click();
  await expect(page.getByRole("heading", { name: "企业详情" })).toBeVisible();
  await page.getByRole("button", { name: "返回" }).click();
  await expect(page.getByRole("heading", { name: "机会详情" })).toBeVisible();
  await page.getByRole("button", { name: "使用长期简历投递" }).click();
  await page.getByRole("button", { name: "查看长期简历" }).click();
  await expect(page.getByRole("heading", { name: "长期简历" })).toBeVisible();
  await page.getByRole("link", { name: "编辑表达" }).first().click();
  await expect(page.getByRole("heading", { name: "个人优势" })).toBeVisible();
...
test("E ended competition hands off to long-term experience and trusted result", async ({ page }) => {
  ...
  await expect(page.getByText("赛事期权限已回收")).toBeVisible();
  await page.getByRole("button", { name: "查看参赛经历" }).click();
  await expect(page.getByRole("heading", { name: "赛事经历" })).toBeVisible();
  await page.getByRole("link", { name: /第十五届三创赛/ }).click();
  await expect(page.getByRole("heading", { name: "参赛经历" })).toBeVisible();
  await page.getByRole("link", { name: /成绩 \/ 成果/ }).click();
  await expect(page.getByRole("heading", { name: "成绩与可信成果" })).toBeVisible();
...
test("unknown path is exposed as an explicit dead-link", async ({ page }) => {
  ...
  await expect(page.getByText("404 / dead-link")).toBeVisible();
