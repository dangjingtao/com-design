...
const repositorySchema = JSON.parse(fs.readFileSync(path.join(designSourceRoot, 'schemas', 'component-contract-v2.schema.json'), 'utf8'));
...
function makeFixture() {
  ...
  fs.writeFileSync(path.join(schemaDir, 'component-contract-v2.schema.json'), JSON.stringify(repositorySchema, null, 2));
  ...
  const writeCatalog = (components) => {
    fs.writeFileSync(path.join(componentDir, 'index.json'), JSON.stringify({
      schemaVersion: 2,
      library: 'com-design',
      version: 'test',
...
test('repository catalog validates every currently indexed Core Component contract', () => {
  ...
  const actualContracts = fs.readdirSync(path.join(designSourceRoot, 'components')).filter((name) => name.endsWith('.json') && name !== 'index.json');
  ...
  assert.equal(result.evidence.previewFiles.length, result.evidence.componentCount);
});
...
test('schema can express intent, states, interaction, accessibility and platform refs without redefining Core semantics', () => {
  const contract = {
    ...
    platformPresentationRefs: [{ ref: 'adapter/select/presentation', context: 'context-capability-driven', note: 'Presentation is selected downstream.' }],
    platformExceptionRefs: [{ ref: 'environment/native-picker', context: 'platform-capability-driven', reason: 'Native picker may materially improve usability.' }],
  };
  assert.deepEqual(validateJsonSchemaValue(contract, repositorySchema), []);
});
...
test('catalog deterministically rejects duplicate component ids and duplicate contract paths', () => {
  ...
  fixture.writeCatalog([entry('button', 'Button'), { ...entry('button', 'Button copy'), name: 'Button copy' }]);
...
test('catalog deterministically rejects broken contract and preview paths', () => {
  const fixture = makeFixture();
  fixture.writeCatalog([{ slug: 'button', name: 'Button', category: 'test', contract: 'components/missing.json', preview: 'preview/missing.html' }]);
  const result = validateComponentCatalog(fixture.root);
...
test('catalog rejects existing but non-canonical preview targets', () => {
  const fixture = makeFixture();
  fixture.writeComponent('button', baseContract('button', 'Button'));
  fixture.writeCatalog([{ ...entry('button', 'Button'), preview: 'components/button.json' }]);
  const result = validateComponentCatalog(fixture.root);
  assert.ok(result.errors.some((error) => error.includes('preview must resolve inside design-source/preview')));
  assert.ok(result.errors.some((error) => error.includes('preview filename must match slug button')));
});

test('catalog rejects wrong preview filename even inside preview directory', () => {
  const fixture = makeFixture();
  fixture.writeComponent('button', baseContract('button', 'Button'));
  fs.writeFileSync(path.join(fixture.previewDir, 'wrong.html'), '<div>wrong</div>\n');
  fixture.writeCatalog([{ ...entry('button', 'Button'), preview: 'preview/wrong.html' }]);
  const result = validateComponentCatalog(fixture.root);
  assert.ok(result.errors.some((error) => error.includes('preview filename must match slug button')));
});
